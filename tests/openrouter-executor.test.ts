...
import { OpenRouterExecutor } from '../src/executors/openrouter-executor.js';
...
describe('OpenRouterExecutor', () => {
  ...
  test('throws if OPENROUTER_API_KEY is missing', () => {
    delete process.env.OPENROUTER_API_KEY;
    expect(() => new OpenRouterExecutor('some/model')).toThrow(/OPENROUTER_API_KEY/);
  });

  test('parses a plain JSON response', async () => {
    global.fetch = mockFetchOnce('{"verdict": "approved", "confidence": 0.9}');
    const exec = new OpenRouterExecutor('google/gemini-2.5-flash');
  ...
  test('clamps an out-of-range string confidence to 1', async () => {
    global.fetch = mockFetchOnce('{"verdict": "approved", "confidence": "85"}');
    const exec = new OpenRouterExecutor('m');
    const { output } = await exec.execute(REVIEWER, {});
    expect(output.confidence).toBe(1); // "85" → 85 → clamped to 1
  });
